...
def topology_tree(k, h, delay=1, **kwargs):
    ...
    print receivers, sources, routers
...
def topology_test(**kwargs):
    seed = 5
    topology = nx.barabasi_albert_graph(500,1,seed=seed)
...
@register_topology_factory('GEANT_TEST')
def topology_geant_test(**kwargs):
    topology = fnss.parse_topology_zoo(path.join(TOPOLOGY_RESOURCES_DIR, 'Geant_test2.graphml')).to_undirected()
    # sources are nodes representing custodian
    sources = [24, 28]
    #sources = [5]
...
def topology_geant(**kwargs):
    ...
    for i in range(cust_no):
    	c1 = deg2.items()[i]
	sources.append(int(c1[0]))
    sources = [4]#,2,34,22,29]
    #print sources
    routers = [v for v in topology.nodes() if v not in sources + receivers]
    #routers = [22, 23, 5, 4, 3, 30, 0, 2, 12, 15, 29, 27, 28, 13, 9, 8, 7, 34, 36]
    #rout = [0, 2, 3, 4, 5, 7, 8, 9, 12, 13, 15, 22, 23, 27, 28, 29, 30, 34, 36]
    #routers = [v for v in rout]
    print receivers,routers
    icr_candidates = routers
    topology.graph['icr_candidates'] = set(icr_candidates)
    for v in sources:
        fnss.add_stack(topology, v, 'source')
...
def topology_garr(**kwargs):
    ...
    topology = list(nx.connected_component_subgraphs(topology))[0]
    deg = nx.degree(topology)
    #receivers = [54, 42, 48, 60, 52, 53, 24, 25, 26, 27, 23, 47, 28, 1, 0, 3, 2, 5, 8, 51, 11, 13, 12, 16, 19, 30, 50]#[v for v in topology.nodes() if deg[v] == 1] 
...
@register_topology_factory('ROCKETFUEL')
def topology_rocketfuel(**kwargs):
    """Return a scenario based on Tiscali topology, parsed from RocketFuel dataset
