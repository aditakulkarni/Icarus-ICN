...
def uniform_cache_placement(topology, cache_budget, **kwargs):
    ...
    cache_budget1 = cache_budget*len(icr_candidates)
    cache_size = iround(cache_budget1/len(icr_candidates))
    print "\nCache: ", cache_size
...
def degree_centrality_cache_placement(topology, cache_budget, **kwargs):
    ...
    cache_budget1 = cache_budget*len(icr_candidates)
    for v in icr_candidates:
        topology.node[v]['stack'][1]['cache_size'] = iround(cache_budget1*deg[v]/total_deg)
...
def betweenness_centrality_cache_placement(topology, cache_budget, **kwargs):
    ...
    cache_budget1 = cache_budget*len(icr_candidates)
    for v in icr_candidates:
        topology.node[v]['stack'][1]['cache_size'] = iround(cache_budget1*betw[v]/total_betw)
...
def uniform_consolidated_cache_placement(topology, cache_budget, spread=0.5,
    ...
    cache_size = iround(cache_budget1/len(target_nodes))
    if cache_size == 0:
        return
