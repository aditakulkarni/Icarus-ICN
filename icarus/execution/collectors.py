...
class LatencyCollector(DataCollector):
    ...
    @inheritdoc(DataCollector)
    def start_session(self, timestamp, receiver, content, increment_count=True):
    ...
    def request_hop(self, u, v, main_path=True):
        if main_path:
            #print "RH u: %d, v: %d, Delay: %d, Sess: %f, Recv: %d, Cont: %d" % (u,v,self.view.link_delay(u, v),self.sess_latency, self.receiver, self.content)
            self.sess_latency += self.view.link_delay(u, v)
    
    @inheritdoc(DataCollector)
    def content_hop(self, u, v, main_path=True):
        if main_path:
            #print "CH u: %d, v: %d, Delay: %d, Sess: %f, Recv: %d, Cont: %d" % (u,v,self.view.link_delay(u, v),self.sess_latency, self.receiver, self.content)
            self.sess_latency += self.view.link_delay(u, v)
    ...
    def end_session(self, success=True):
        ...
        #print "End sess lat: ",self.latency
    
    @inheritdoc(DataCollector)
    def results(self):
        results = Tree({'MEAN': self.latency/self.sess_count})
        print "Lat:", self.latency
        print "Sess cnt:", self.sess_count
        print "Sess lat:", self.latency/self.sess_count
        if self.cdf:
